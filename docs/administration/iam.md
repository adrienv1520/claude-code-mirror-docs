...
1. Follow the [Bedrock docs](../deployment/amazon-bedrock.md) or [Vertex docs](../deployment/google-vertex-ai.md)
2. Distribute the environment variables and instructions for generating cloud credentials to your users. Read more about how to [manage configuration here](../configuration/settings.md).
...
Claude Code supports several permission modes that can be set as the `defaultMode` in [settings files](../configuration/settings.md#settings-files):
...
* **Persistent configuration**: Add to `additionalDirectories` in [settings files](../configuration/settings.md#settings-files)
...
[Claude Code hooks](../build-with-claude-code/hooks-guide.md) provide a way to register custom shell commands to perform permission evaluation at runtime. When Claude Code makes a tool call, PreToolUse hooks run before the permission system runs, and the hook output can determine whether to approve or deny the tool call in place of the permission system.
...
These policy files follow the same format as regular [settings files](../configuration/settings.md#settings-files) but cannot be overridden by user or project settings. This ensures consistent security policies across your organization.
...
* **Custom credential scripts**: The [`apiKeyHelper`](../configuration/settings.md#available-settings) setting can be configured to run a shell script that returns an API key.
